...
function isDirectionPredicate(fn: any): fn is MovingDirectionUtils.Predicate {
...
export class StepByStepNavigationMode extends NavigationMode {
  ...
  goToStep(destinationIndex: number, preFinalize?: EventEmitter<void>, postFinalize?: EventEmitter<void>): void {
    this.canGoToStep(destinationIndex).then(navigationAllowed => {
      if (navigationAllowed) {
        ...
        // leave current step
        this.wizardState.currentStep.completed = this.wizardState.currentStep.completed || movingDirection === MovingDirection.Forwards;
  ...
  isNavigable(destinationIndex: number): boolean {
    ...
    if (isBoolean(currentStep.canExit) && !currentStep.canExit) {
      ...
      const canExitCurrent = currentStep.canExit(movingDirection);
      if (isBoolean(canExitCurrent) && !canExitCurrent) {
        return false;
      }
    }
    const destStep = this.wizardState.getStepAtIndex(destinationIndex);
    if (isBoolean(destStep.canEnter) && !destStep.canEnter) {
      return false;
    } else if (isDirectionPredicate(currentStep.canEnter)) {
      const canEnterDest = currentStep.canEnter(movingDirection);
      if (isBoolean(canEnterDest) && !canEnterDest) {
        return false;
      }
    }
    if (destStep instanceof WizardCompletionStep) {
